...
function initDb() {
  ...
  db.run(`
    CREATE TABLE IF NOT EXISTS manager_artists (
      ...
      publisher_name TEXT,
      publisher_ipi TEXT,
      soundexchange_registered INTEGER DEFAULT 0,
      mlc_registered INTEGER DEFAULT 0,
      songtrust_registered INTEGER DEFAULT 0,
      musixmatch_verified INTEGER DEFAULT 0,
      spotify_url TEXT,
  ...
  // --- Manager: Catalog ---
  db.run(`
    CREATE TABLE IF NOT EXISTS manager_catalog (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      ...
      bmi_registered INTEGER DEFAULT 0,
      bmi_work_number TEXT,
      soundexchange_claimed INTEGER DEFAULT 0,
      mlc_registered INTEGER DEFAULT 0,
      musixmatch_lyrics INTEGER DEFAULT 0,
      songtrust_registered INTEGER DEFAULT 0,
      has_plain_lyrics INTEGER DEFAULT 0,
      ...
      updated_at TEXT DEFAULT (datetime('now')),
      deleted_at TEXT,
      FOREIGN KEY (artist_id) REFERENCES manager_artists(id)
    )
  `);
  db.run('CREATE INDEX IF NOT EXISTS idx_mc_artist ON manager_catalog(artist_id)');
  db.run('CREATE INDEX IF NOT EXISTS idx_mc_isrc ON manager_catalog(isrc)');
  ...
  db.run(`
    CREATE TABLE IF NOT EXISTS manager_splits (
      ...
      writer_legal_name TEXT NOT NULL,
      writer_role TEXT,
      writer_pro TEXT,
      writer_ipi TEXT,
      writer_pct REAL NOT NULL,
      publisher_name TEXT,
      publisher_pct REAL,
      created_at TEXT DEFAULT (datetime('now')),
      deleted_at TEXT,
      FOREIGN KEY (catalog_id) REFERENCES manager_catalog(id)
    )
  `);
  db.run('CREATE INDEX IF NOT EXISTS idx_ms_catalog ON manager_splits(catalog_id)');

  // --- Manager: Royalties ---
  db.run(`
    CREATE TABLE IF NOT EXISTS manager_royalties (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      artist_id INTEGER NOT NULL,
  ...
  `);
  db.run('CREATE INDEX IF NOT EXISTS idx_mr_artist ON manager_royalties(artist_id)');
  ...
  `);
  db.run('CREATE INDEX IF NOT EXISTS idx_mt_artist ON manager_tasks(artist_id)');
  db.run('CREATE INDEX IF NOT EXISTS idx_mt_status ON manager_tasks(status)');
...
// All behind requireAdmin. Zero impact on existing routes.
// ============================================================

// --- Manager: Artists CRUD ---
app.get('/api/manager/artists', requireAdmin, (req, res) => {
  const rows = dbHelpers.prepare(`SELECT * FROM manager_artists WHERE deleted_at IS NULL ORDER BY artist_name`).all();
  res.json(rows);
});

app.post('/api/manager/artists', requireAdmin, (req, res) => {
  const { artist_name, artist_legal_name, label, distributor, pro, pro_member_id, writer_ipi, publisher_name, publisher_ipi, spotify_url, apple_music_url, youtube_url, instagram, location, management_pct, notes } = req.body;
  if (!artist_name) return res.status(400).json({ error: 'artist_name required' });
  const result = dbHelpers.prepare(`
    INSERT INTO manager_artists (manager_id, artist_name, artist_legal_name, label, distributor, pro, pro_member_id, writer_ipi, publisher_name, publisher_ipi, spotify_url, apple_music_url, youtube_url, instagram, location, management_pct, notes)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(req.session.userId, artist_name, artist_legal_name || null, label || null, distributor || null, pro || null, pro_member_id || null, writer_ipi || null, publisher_name || null, publisher_ipi || null, spotify_url || null, apple_music_url || null, youtube_url || null, instagram || null, location || null, management_pct || null, notes || null);
  logOperation(req, 'manager_artist_create', 'manager_artist', result.lastInsertRowid, { artist_name });
  flushDbNow();
  res.json({ id: result.lastInsertRowid, artist_name });
});

app.get('/api/manager/artists/:id', requireAdmin, (req, res) => {
  const row = dbHelpers.prepare(`SELECT * FROM manager_artists WHERE id = ? AND deleted_at IS NULL`).get(req.params.id);
  if (!row) return res.status(404).json({ error: 'Artist not found' });
  res.json(row);
});

app.put('/api/manager/artists/:id', requireAdmin, (req, res) => {
  const fields = ['artist_name','artist_legal_name','label','distributor','pro','pro_member_id','writer_ipi','publisher_name','publisher_ipi','soundexchange_registered','mlc_registered','songtrust_registered','musixmatch_verified','spotify_url','apple_music_url','youtube_url','instagram','location','management_pct','notes'];
  ...
  dbHelpers.prepare(`UPDATE manager_artists SET ${sets.join(', ')} WHERE id = ? AND deleted_at IS NULL`).run(...vals);
  logOperation(req, 'manager_artist_update', 'manager_artist', req.params.id, req.body);
  flushDbNow();
  res.json({ ok: true });
});

app.delete('/api/manager/artists/:id', requireAdmin, (req, res) => {
  dbHelpers.prepare(`UPDATE manager_artists SET deleted_at = datetime('now') WHERE id = ?`).run(req.params.id);
  logOperation(req, 'manager_artist_delete', 'manager_artist', req.params.id);
  flushDbNow();
  res.json({ ok: true });
});

// --- Manager: Catalog CRUD ---
app.get('/api/manager/artists/:id/catalog', requireAdmin, (req, res) => {
  const rows = dbHelpers.prepare(`SELECT * FROM manager_catalog WHERE artist_id = ? AND deleted_at IS NULL ORDER BY release_date DESC`).all(req.params.id);
  res.json(rows);
});

app.post('/api/manager/artists/:id/catalog', requireAdmin, (req, res) => {
  const { song_title, release_type, album_name, release_date, isrc, upc, featured_artists, producer, duration, spotify_streams, dk_split_pct, notes } = req.body;
  if (!song_title) return res.status(400).json({ error: 'song_title required' });
  const result = dbHelpers.prepare(`
    INSERT INTO manager_catalog (artist_id, song_title, release_type, album_name, release_date, isrc, upc, featured_artists, producer, duration, spotify_streams, dk_split_pct, notes)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(req.params.id, song_title, release_type || null, album_name || null, release_date || null, isrc || null, upc || null, featured_artists || null, producer || null, duration || null, spotify_streams || null, dk_split_pct || null, notes || null);
  logOperation(req, 'manager_catalog_create', 'manager_catalog', result.lastInsertRowid, { song_title, artist_id: req.params.id });
  flushDbNow();
  res.json({ id: result.lastInsertRowid, song_title });
});

app.post('/api/manager/artists/:id/catalog/bulk', requireAdmin, (req, res) => {
  const { songs } = req.body;
  if (!Array.isArray(songs) || !songs.length) return res.status(400).json({ error: 'songs array required' });
  const ids = [];
  for (const s of songs) {
    if (!s.song_title) continue;
    const result = dbHelpers.prepare(`
      INSERT INTO manager_catalog (artist_id, song_title, release_type, album_name, release_date, isrc, upc, featured_artists, producer, duration, spotify_streams, dk_split_pct, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(req.params.id, s.song_title, s.release_type || null, s.album_name || null, s.release_date || null, s.isrc || null, s.upc || null, s.featured_artists || null, s.producer || null, s.duration || null, s.spotify_streams || null, s.dk_split_pct || null, s.notes || null);
    ids.push(result.lastInsertRowid);
  }
  logOperation(req, 'manager_catalog_bulk_create', 'manager_catalog', null, { artist_id: req.params.id, count: ids.length });
  flushDbNow();
  res.json({ imported: ids.length, ids });
});

app.put('/api/manager/catalog/:songId', requireAdmin, (req, res) => {
  const fields = ['song_title','release_type','album_name','release_date','isrc','upc','featured_artists','producer','duration','spotify_streams','bmi_registered','bmi_work_number','soundexchange_claimed','mlc_registered','musixmatch_lyrics','songtrust_registered','has_plain_lyrics','has_synced_lyrics','dk_split_pct','notes'];
  const sets = []; const vals = [];
  for (const f of fields) {
    if (req.body[f] !== undefined) { sets.push(`${f} = ?`); vals.push(req.body[f]); }
  }
  if (!sets.length) return res.status(400).json({ error: 'No fields to update' });
  sets.push("updated_at = datetime('now')");
  vals.push(req.params.songId);
  dbHelpers.prepare(`UPDATE manager_catalog SET ${sets.join(', ')} WHERE id = ? AND deleted_at IS NULL`).run(...vals);
  logOperation(req, 'manager_catalog_update', 'manager_catalog', req.params.songId, req.body);
  flushDbNow();
  res.json({ ok: true });
});

app.delete('/api/manager/catalog/:songId', requireAdmin, (req, res) => {
  dbHelpers.prepare(`UPDATE manager_catalog SET deleted_at = datetime('now') WHERE id = ?`).run(req.params.songId);
  logOperation(req, 'manager_catalog_delete', 'manager_catalog', req.params.songId);
  flushDbNow();
  res.json({ ok: true });
});

// --- Manager: Registration Dashboard + Bulk Register ---
app.get('/api/manager/catalog/gaps', requireAdmin, (req, res) => {
  const rows = dbHelpers.prepare(`
    SELECT mc.*, ma.artist_name FROM manager_catalog mc
    JOIN manager_artists ma ON mc.artist_id = ma.id
    WHERE mc.deleted_at IS NULL AND ma.deleted_at IS NULL
    AND (mc.bmi_registered = 0 OR mc.soundexchange_claimed = 0 OR mc.mlc_registered = 0 OR mc.musixmatch_lyrics = 0)
    ORDER BY mc.spotify_streams DESC NULLS LAST
  `).all();
  res.json(rows);
});

app.get('/api/manager/registration/dashboard', requireAdmin, (req, res) => {
  const total = dbHelpers.prepare(`SELECT COUNT(*) as cnt FROM manager_catalog WHERE deleted_at IS NULL`).get().cnt;
  const bmi = dbHelpers.prepare(`SELECT COUNT(*) as cnt FROM manager_catalog WHERE deleted_at IS NULL AND bmi_registered = 1`).get().cnt;
  const se = dbHelpers.prepare(`SELECT COUNT(*) as cnt FROM manager_catalog WHERE deleted_at IS NULL AND soundexchange_claimed = 1`).get().cnt;
  const mlc = dbHelpers.prepare(`SELECT COUNT(*) as cnt FROM manager_catalog WHERE deleted_at IS NULL AND mlc_registered = 1`).get().cnt;
  const mm = dbHelpers.prepare(`SELECT COUNT(*) as cnt FROM manager_catalog WHERE deleted_at IS NULL AND musixmatch_lyrics = 1`).get().cnt;
  res.json({ total, bmi, soundexchange: se, mlc, musixmatch: mm });
});

app.put('/api/manager/catalog/:songId/register', requireAdmin, (req, res) => {
  const { platform, registered } = req.body;
  const map = { bmi: 'bmi_registered', soundexchange: 'soundexchange_claimed', mlc: 'mlc_registered', musixmatch: 'musixmatch_lyrics', songtrust: 'songtrust_registered' };
  const col = map[platform];
  if (!col) return res.status(400).json({ error: 'Invalid platform' });
  dbHelpers.prepare(`UPDATE manager_catalog SET ${col} = ?, updated_at = datetime('now') WHERE id = ? AND deleted_at IS NULL`).run(registered ? 1 : 0, req.params.songId);
  logOperation(req, 'manager_register', 'manager_catalog', req.params.songId, { platform, registered });
  flushDbNow();
  res.json({ ok: true });
});

app.post('/api/manager/catalog/bulk-register', requireAdmin, (req, res) => {
  const { song_ids, platform, registered } = req.body;
  const map = { bmi: 'bmi_registered', soundexchange: 'soundexchange_claimed', mlc: 'mlc_registered', musixmatch: 'musixmatch_lyrics', songtrust: 'songtrust_registered' };
  const col = map[platform];
  if (!col || !Array.isArray(song_ids)) return res.status(400).json({ error: 'Invalid request' });
  for (const id of song_ids) {
    dbHelpers.prepare(`UPDATE manager_catalog SET ${col} = ?, updated_at = datetime('now') WHERE id = ? AND deleted_at IS NULL`).run(registered ? 1 : 0, id);
  }
  logOperation(req, 'manager_bulk_register', 'manager_catalog', null, { platform, registered, count: song_ids.length });
  flushDbNow();
  res.json({ ok: true, updated: song_ids.length });
});

// --- Manager: Splits ---
app.get('/api/manager/catalog/:songId/splits', requireAdmin, (req, res) => {
  const rows = dbHelpers.prepare(`SELECT * FROM manager_splits WHERE catalog_id = ? AND deleted_at IS NULL`).all(req.params.songId);
  res.json(rows);
});

app.post('/api/manager/catalog/:songId/splits', requireAdmin, (req, res) => {
  const { splits } = req.body;
  if (!Array.isArray(splits)) return res.status(400).json({ error: 'splits array required' });
  // Replace existing splits for this song
  dbHelpers.prepare(`UPDATE manager_splits SET deleted_at = datetime('now') WHERE catalog_id = ?`).run(req.params.songId);
  for (const sp of splits) {
    if (!sp.writer_legal_name || sp.writer_pct == null) continue;
    dbHelpers.prepare(`
      INSERT INTO manager_splits (catalog_id, writer_legal_name, writer_role, writer_pro, writer_ipi, writer_pct, publisher_name, publisher_pct)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(req.params.songId, sp.writer_legal_name, sp.writer_role || null, sp.writer_pro || null, sp.writer_ipi || null, sp.writer_pct, sp.publisher_name || null, sp.publisher_pct || null);
  }
  logOperation(req, 'manager_splits_update', 'manager_catalog', req.params.songId, { count: splits.length });
  flushDbNow();
  res.json({ ok: true });
});

// --- Manager: Royalties ---
app.get('/api/manager/royalties', requireAdmin, (req, res) => {
  const rows = dbHelpers.prepare(`
    SELECT mr.*, ma.artist_name FROM manager_royalties mr
    JOIN manager_artists ma ON mr.artist_id = ma.id
    WHERE mr.deleted_at IS NULL ORDER BY mr.created_at DESC
  `).all();
  res.json(rows);
});

app.post('/api/manager/royalties', requireAdmin, (req, res) => {
  const { artist_id, platform, period, amount_cents, statement_notes } = req.body;
  if (!artist_id || !platform || !period || amount_cents == null) return res.status(400).json({ error: 'artist_id, platform, period, amount_cents required' });
  const result = dbHelpers.prepare(`
    INSERT INTO manager_royalties (artist_id, platform, period, amount_cents, statement_notes)
    VALUES (?, ?, ?, ?, ?)
  `).run(artist_id, platform, period, amount_cents, statement_notes || null);
  logOperation(req, 'manager_royalty_create', 'manager_royalty', result.lastInsertRowid, { artist_id, platform, period, amount_cents });
  flushDbNow();
  res.json({ id: result.lastInsertRowid });
});

app.get('/api/manager/royalties/summary', requireAdmin, (req, res) => {
  const byArtist = dbHelpers.prepare(`
    SELECT ma.artist_name, SUM(mr.amount_cents) as total_cents FROM manager_royalties mr
    JOIN manager_artists ma ON mr.artist_id = ma.id
    WHERE mr.deleted_at IS NULL GROUP BY mr.artist_id
  `).all();
  const byPlatform = dbHelpers.prepare(`
    SELECT platform, SUM(amount_cents) as total_cents FROM manager_royalties
    WHERE deleted_at IS NULL GROUP BY platform
  `).all();
  res.json({ byArtist, byPlatform });
});

// --- Manager: Tasks ---
app.get('/api/manager/tasks', requireAdmin, (req, res) => {
  const rows = dbHelpers.prepare(`
    SELECT mt.*, ma.artist_name, mc.song_title FROM manager_tasks mt
    JOIN manager_artists ma ON mt.artist_id = ma.id
    LEFT JOIN manager_catalog mc ON mt.catalog_id = mc.id
    WHERE mt.deleted_at IS NULL ORDER BY mt.priority DESC, mt.created_at
  `).all();
  res.json(rows);
});

app.post('/api/manager/tasks', requireAdmin, (req, res) => {
  const { artist_id, catalog_id, task_type, description, priority, due_date, notes } = req.body;
  if (!artist_id || !task_type || !description) return res.status(400).json({ error: 'artist_id, task_type, description required' });
  const result = dbHelpers.prepare(`
    INSERT INTO manager_tasks (artist_id, catalog_id, task_type, description, priority, due_date, notes)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(artist_id, catalog_id || null, task_type, description, priority || 0, due_date || null, notes || null);
  logOperation(req, 'manager_task_create', 'manager_task', result.lastInsertRowid, { task_type, description });
  flushDbNow();
  res.json({ id: result.lastInsertRowid });
});

app.put('/api/manager/tasks/:id', requireAdmin, (req, res) => {
  ...
  sets.push("updated_at = datetime('now')");
  vals.push(req.params.id);
  dbHelpers.prepare(`UPDATE manager_tasks SET ${sets.join(', ')} WHERE id = ? AND deleted_at IS NULL`).run(...vals);
  logOperation(req, 'manager_task_update', 'manager_task', req.params.id, req.body);
  flushDbNow();
  res.json({ ok: true });
});

app.post('/api/manager/tasks/auto-generate', requireAdmin, (req, res) => {
  const { artist_id } = req.body;
  if (!artist_id) return res.status(400).json({ error: 'artist_id required' });
  const gaps = dbHelpers.prepare(`SELECT * FROM manager_catalog WHERE artist_id = ? AND deleted_at IS NULL`).all(artist_id);
  let created = 0;
  const platforms = [
    { col: 'bmi_registered', name: 'BMI', type: 'register_bmi' },
    { col: 'soundexchange_claimed', name: 'SoundExchange', type: 'register_soundexchange' },
    { col: 'mlc_registered', name: 'MLC', type: 'register_mlc' },
    { col: 'musixmatch_lyrics', name: 'Musixmatch', type: 'register_musixmatch' },
  ];
  for (const song of gaps) {
    for (const p of platforms) {
      if (song[p.col] === 0) {
        // Check if task already exists
        const existing = dbHelpers.prepare(`SELECT id FROM manager_tasks WHERE catalog_id = ? AND task_type = ? AND deleted_at IS NULL AND status != 'completed'`).get(song.id, p.type);
        if (!existing) {
          dbHelpers.prepare(`INSERT INTO manager_tasks (artist_id, catalog_id, task_type, description, priority) VALUES (?, ?, ?, ?, ?)`).run(
            artist_id, song.id, p.type, `Register "${song.song_title}" on ${p.name}`, song.spotify_streams ? 2 : 1
          );
          created++;
        }
      }
  ...
  logOperation(req, 'manager_tasks_auto_generate', 'manager_task', null, { artist_id, created });
  flushDbNow();
  res.json({ created });
});
